...
set outpsfile=研究区底图.ps
set ImageSize=14c
set PreviewSize=3c
set PreviewMove_X=11c
set PsTextPath=构造及其他的地址单元的坐标与标注
del %outpsfile%
rem -------------------------------------------------------底图绘制------------------------------------------------------------------------
rem 1. basemap
...
rem 3. 洋中脊
gmt psxy 数据\SWIR分段\BTJ-SH_SWIR.txt -R -J -W1p,ROYALBLUE -A -O -K>> %outpsfile%
gmt psxy 数据\SWIR分段\SH-16E_SWIR斜向超级扩张段.txt -R -J -W1p,MAGENTA1 -A -O -K>> %outpsfile%
gmt psxy 数据\SWIR分段\16E-25E_SWIR正向超级扩张段.txt -R -J -W1p,ROYALBLUE -A -O -K>> %outpsfile%
gmt psxy 数据\SWIR分段\DT-PE_第四段_SWIR.txt -R -J -W1p,MAGENTA1 -A -O -K>> %outpsfile%
gmt psxy 数据\SWIR分段\ES-GA_SWIR第五段.txt -R -J -W1p,ROYALBLUE -A -O -K>> %outpsfile%
gmt psxy 数据\SWIR分段\GA-64E_SWIR第六段.txt -R -J -W1p,MAGENTA1 -A -O -K>> %outpsfile%
gmt psxy 数据\CIR脊轴.txt -R -J -W1p,red -A -O -K>> %outpsfile%
gmt psxy 数据\SEIR脊轴.txt -R -J -W1p,red2 -A -O -K>> %outpsfile%
gmt psxy 数据\AAR脊轴.txt -R -J -W1p,red2 -A -O -K>> %outpsfile%
gmt psxy 数据\MAR脊轴.txt -R -J -W1p,red2 -A -O -K>> %outpsfile%
REM 4. 断裂分布
...
rem 添加主要的地标的符号
gmt psxy -R -J 数据\全球热点分布.dat -St0.5p -Gred -W0.1c,red -O -K -V >> %outpsfile%
...
rem 添加板块边界
gmt psxy 数据\来自GPlates的数据\Seton_etal_ESR2012_Ridges_2012.1.xy -R -J -W0.2p,red -A -O -K>> %outpsfile%
gmt psxy -R -J 数据\全球热点分布.dat -St0.8p -Gred -W0.01p,red -O -K -V >> %outpsfile%

rem 海岸线+陆地
gmt pscoast -R -J -A50000 -W0.1 -Glightblue -Di -O -K >> %outpsfile%
gmt psxy 研究区域范围.txt -R -J -W0.5p,blue -L -A -O -K>> %outpsfile%
rem ----------------------------------添加大区域缩略图（这个务必放在最后）--------------------------------------------------------------------
del gmt.conf *.history tmp